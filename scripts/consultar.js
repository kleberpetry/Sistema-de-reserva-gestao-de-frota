...
$(document).on("change", ".datetime-input", function () {
  ...
  let veiculosDisponiveis = $('#veiculo option').map(function() {
    return $(this).text();
  }).get();

  if (valores.length >= 4) {
    // Habilitar select de veículos, contendo apenas os veículos disponíveis
    ...
    events.forEach(event => {
      ...
      //const veiculo = event.title.split('-')[0].trim();
      const placa = event.title.split(': ')[1].split(' -')[0].trim();

      const eventStart = startDate.getTime();
      const eventEnd = endDate.getTime();
      const newReservationStart = dataSaidaInput.getTime();
      const newReservationEnd = dataRetornoInput.getTime();

      if ((newReservationStart >= eventStart && newReservationStart < eventEnd) ||
          (newReservationEnd > eventStart && newReservationEnd <= eventEnd) ||
          (newReservationStart <= eventStart && newReservationEnd >= eventEnd)) {
            veiculosDisponiveis = veiculosDisponiveis.filter(v => !v.includes(placa));
            //veiculosDisponiveis = veiculosDisponiveis.filter(v => !v.includes(veiculo));
      }
    });
  } else {
    $('#veiculo').prop('disabled', true);

    var element = $('#veiculos');
    var opt = element.find('option:contains(' + 'Selecione um veículo' + ')');

    element.find('option').prop('selected', false);
    opt.prop('selected', true);
  }

  $('#veiculo option').each(function() {
    var label = $(this).text();

    if (veiculosDisponiveis.includes(label)) {
      $(this).prop('disabled', false);
    } else {
      $(this).prop('disabled', true);
    }

    if (label == 'Selecione um veículo') {
      $(this).prop('disabled', true);
    }
  });
...
$(document).on("click", "#finalizar-reserva", function() {
  ...
  if (!editing) {
    events.forEach(event => {
      ...
      //if (event.title.includes(veiculo.split('-')[0].trim())) {
      if (event.title.includes(veiculo.split('-')[1].trim())) {
        const eventStart = startDate.getTime();
        const eventEnd = endDate.getTime();
        const newReservationStart = dataSaidaInput.getTime();
        const newReservationEnd = dataRetornoInput.getTime();
  ...
  $.ajax({
    ...
    success: function(reserva) {
      ...
      calendar.addEvent({
        ...
        title: `${reserva[0].modelo} - placa: ${reserva[0].placa} - condutor: ${reserva[0].nome} - destino: ${reserva[0].destino}`,
